...
check_dependencies() {
	local commands='phpunit-skelgen sed find tr'
	local result=0
...
group=$(get_group ${RELATIVE_CLASS_PATH})

regex=$(echo 's=(Generated by PHPUnit_SkeletonGenerator.*)\s=\1\n \* \@group '${group}'\n=')
perl -pi -e "${regex}" "${RELATIVE_TEST_PATH}"
